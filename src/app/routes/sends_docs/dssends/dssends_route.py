...
from .schemas import DsSendsModel, DsSendsFilterModel, DsSendsFilterNumberModel, allSupportDocsModel, api
...
class All(Resource):
    ...
    @api.expect(allSupportDocsModel)
    def post(self):
        """

        Consulta envios de documento soporte almacenadas en la base de datos por numero y tamaño de pagina
        """
        data = request.json
...
class GetByid(Resource):
    ...
    def get(self, id):
        ...
        Obtiene un envio de documento soporte dado su id
        """
...
class Pdf(Resource):
    ...
    @api.doc('pdf')
    @api.expect(DsSendsFilterNumberModel)
    def post(self):
        """
...
@api.response(201, 'Envio de documento soporte creado correctamente')
class NewData(Resource):
    decorators = [auth_token.login_required]

    @api.doc('create_new_dssend')
    @api.expect(DsSendsModel)
    def post(self):
        """
        Crea un nuevo envio de Documento Soporte
        """
...
@api.response(201, 'Envio de documento soporte actualizado correctamente')
class UpdateData(Resource):
    decorators = [auth_token.login_required]

    @api.doc('update_dssend')
    @api.marshal_with(DsSendsModel)
    def put(self):
        """

        Actualiza un envio de Documento Soporte
        """
        data = request.json
