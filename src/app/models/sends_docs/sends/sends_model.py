...
class SendsModel(BaseModel):
    __tablename__ = 'sendsinvoices'
    ...
    def get_data(export: bool = False, pageNumber=0, pageSize=None):
        try:
            if pageSize != None:
                response = db.session.query(SendsModel) \
                    .limit(int(pageSize)) \
                ...
                response = db.session.query(SendsModel) \
                    .all()
            if export:
                response = [SendsModel.export_data(x) for x in response]
            return response
        except Exception as e:
            print(e)
            response = ResponseData.internal_server_error(str(e))
    ...
    def get_by_id(id: int, export: bool = False):
        try:
            response = db.session.query(SendsModel) \
                .filter(SendsModel.Id == id) \
                .first()
            if export and response:
                response = SendsModel.export_data(response)
            return response
        except Exception as e:
            print(e)
            response = ResponseData.internal_server_error(str(e))
        ...
        try:
            ...
            if pageSize != None:
                if StartDate != "":
                    response = db.session.query(SendsModel) \
                        .filter(
                        SendsModel.IssuerNit == IssuerNit, SendsModel.Prefix + SendsModel.SerieNumber >= StartNumber
                        , SendsModel.Prefix + SendsModel.SerieNumber <= EndNumber, SendsModel.ClientNit >= StartClientNit,
                            SendsModel.ClientNit <= EndClientNit,
                            SendsModel.DateSend >= StartDate,  SendsModel.DateSend <= EndDate
                        , SendsModel.Status == Status, SendsModel.DocType == DocType) \
                        .limit(int(pageSize)) \
                        .offset((int(pageNumber) - 1) * int(pageSize))\
                        .all()
                else:
                ...
                if StartDate != "":
                    response = db.session.query(SendsModel) \
                        .filter(
                        SendsModel.IssuerNit == IssuerNit, SendsModel.Prefix + SendsModel.SerieNumber >= StartNumber
                        , SendsModel.Prefix + SendsModel.SerieNumber <= EndNumber,
                        SendsModel.ClientNit >= StartClientNit,
                        SendsModel.ClientNit <= EndClientNit,
                        SendsModel.DateSend >= StartDate, SendsModel.DateSend <= EndDate
                        , SendsModel.Status == Status, SendsModel.DocType == DocType) \
                        .all()
            ...
            if export and response:
                response = [SendsModel.export_data_2(x) for x in response]
            return response
        except Exception as e:
            print(e)
            response = ResponseData.internal_server_error(str(e))
    ...
    def get_by_number(Nit: str, SeriePrefix: str, SerieNumber: str, DocType: str, export: bool = False):
        try:
            response = db.session.query(SendsModel) \
                .filter(
                SendsModel.IssuerNit == Nit, SendsModel.Prefix == SeriePrefix, SendsModel.SerieNumber == SerieNumber
                , SendsModel.Status == "1", SendsModel.DocType == DocType) \
                .first()
            if export and response:
                response = SendsModel.export_data(response)
            return response
    ...
    def new_data(data: dict):
        try:
            ...
            response = {
                'data': SendsModel.export_data(new_data),
                'statusCode': 201,
    ...
    def update_data(data: dict):
        try:
            update_data = SendsModel.get_by_id(id=data['Id'], export=False)
            if not update_data:
                return NotFound('Los datos ingresados no pertenecen o no son validos para el registro consultado')
            update_data = SendsModel.import_data(update_data, data=data)
            SendsModel.save(update_data, create=True, commit=True)
            response = {
                'data': SendsModel.export_data(update_data),
                'statusCode': 200,
