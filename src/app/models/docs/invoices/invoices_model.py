...
class InvoicesModel(BaseModel):
    ...
    DocType = db.Column(db.String(5), nullable=False, comment='Tipo de Doc AD=Contenedor FV=factura NC=Nota '
                                                              'Cr ND=Nota DB DS=Documento Soporte NOM=Nomina '
                                                              'NOMA=Nomina Ajuste ')
    ...
    InvoiceAuthorization = db.Column(db.String(14), nullable=False, comment='Resolucion DIAN ')
    ...
    DianTestSetId = db.Column(db.String(100), nullable=False, comment='set de prueba')

    InvoiceTypeCode = db.Column(db.String(2), nullable=False, comment='tipo de documento tabla 13.1.3 ')
    ...
    CreditNoteId = db.Column(db.String(20), nullable=True, comment='nota credito origen ')
    CreditNoteIssueDate = db.Column(db.String(10), nullable=True, comment='fecha nota credito ')
    CreditNoteCude = db.Column(db.String(96), nullable=True, comment='cude nota credito ')
    DebitNoteId = db.Column(db.String(20), nullable=True, comment='nota debito origen ')
    DebitNoteIssueDate = db.Column(db.String(10), nullable=True, comment='fecha nota debito ')
    DebitNoteCude = db.Column(db.String(96), nullable=True, comment='cude nota debito ')
    ...
    ProfileExecutionId = db.Column(db.String(1), nullable=False, comment='tipo de ambiente 1=Produccion 2=Habilitacion')
    Uuid = db.Column(db.String(100), nullable=True, comment='cude ')
    ...
    ActualDeliveryDate = db.Column(db.String(10), nullable=True, comment='nit vendedor ')
    ActualDeliveryTime = db.Column(db.String(10), nullable=True, comment='nit vendedor ')
    ...
    def export_data(self):
        return {
            ...
            'DianTestSetId': self.DianTestSetId,
            'InvoiceTypeCode': self.InvoiceTypeCode,
            'OperationType': self.OperationType,
            'OrderReferenceId': self.OrderReferenceId,
            'OrderReferenceIssueDate': self.OrderReferenceIssueDate,
            'CreditNoteId': self.CreditNoteId,
            'CreditNoteIssueDate': self.CreditNoteIssueDate,
            'CreditNoteCude': self.CreditNoteCude,
            'DebitNoteId': self.DebitNoteId,
            'DebitNoteIssueDate': self.DebitNoteIssueDate,
            'DebitNoteCude': self.DebitNoteCude,
            ...
            'TotalIva': self.TotalIva,
            'TotalTaxConsume': self.TotalTaxConsume,
            'TotalIca': self.TotalIca,
    ...
    def import_data(self, data):
        ...
        if 'DianTestSetId' in data:
            self.DianTestSetId = data['DianTestSetId']
        if 'InvoiceTypeCode' in data:
            self.InvoiceTypeCode = data['InvoiceTypeCode']
        ...
        if 'CreditNoteId' in data:
            self.CreditNoteId = data['CreditNoteId']
        if 'CreditNoteIssueDate' in data:
            self.CreditNoteIssueDate = data['CreditNoteIssueDate']
        if 'CreditNoteCude' in data:
            self.CreditNoteCude = data['CreditNoteCude']
        if 'DebitNoteId' in data:
            self.DebitNoteId = data['DebitNoteId']
        if 'DebitNoteIssueDate' in data:
            self.DebitNoteIssueDate = data['DebitNoteIssueDate']
        if 'DebitNoteCude' in data:
            self.DebitNoteCude = data['DebitNoteCude']
        ...
        if 'TotalIva' in data:
            self.TotalIva = data['TotalIva']
        if 'TotalTaxConsume' in data:
            self.TotalTaxConsume = data['TotalTaxConsume']
        if 'TotalIca' in data:
            self.TotalIca = data['TotalIca']
    ...
    def get_data(export: bool = False, pageNumber=0, pageSize=None):
        try:
            if pageSize != None:
                response = db.session.query(InvoicesModel) \
                    .limit(int(pageSize)) \
                ...
                response = db.session.query(InvoicesModel) \
                    .all()
            if export:
                response = [InvoicesModel.export_data(x) for x in response]
            return response
    ...
    def get_by_id(id: int, export: bool = False):
        try:
            response = db.session.query(InvoicesModel) \
                .filter(InvoicesModel.Id == id) \
                .first()
            if export and response:
                response = InvoicesModel.export_data(response)
            return response
        except Exception as e:
            print(e)
            response = ResponseData.internal_server_error(str(e))
    ...
    @staticmethod
    def Create_xml_invoices(new_data: dict):
        ...
        TaxHoldingTotalGlobal = None
        TaxHoldingSubTotalGlobal = None

        if "WithholdingTaxSubTotals" in new_data:
            TaxHoldingSubTotalGlobal = new_data["WithholdingTaxSubTotals"]
        if "WithholdingTaxTotals" in new_data:
            TaxHoldingTotalGlobal = new_data["WithholdingTaxTotals"]
        ...
        if new_data['CUFE'] == '':
            if DocType == "FV" and Contingency == "N":
                CUFE = (SeriePrefix + SerieNumber + IssueDate + IssueTime + str(InvoiceVal) + '01' + str(TotalIva) +
                        '04' + str(TotalTaxConsume) + '03' + str(TotalIca) + str(TotalVal) + IssuerNit +
                        CustomerDocumentNumber + TechnicalKey + ProfileExecutionId)
                h = hashlib.sha384()
                ...
                CUFE = (SeriePrefix + SerieNumber + IssueDate + IssueTime + str(InvoiceVal) + '01' + str(TotalIva) +
                        '04' + str(TotalTaxConsume) + '03' + str(TotalIca) + str(TotalVal) + IssuerNit +
                        CustomerDocumentNumber + Pin + ProfileExecutionId)
                h = hashlib.sha384()
                h.update(CUFE.encode('utf-8'))
                CUFE = h.hexdigest()
            new_data["CUFE"] = CUFE
        ...
        if ProfileExecutionId == "1":
            QRCode = "https://catalogo-vpfe.dian.gov.co/document/searchqr?documentkey=" + str(CUFE)
        else:
            QRCode = "https://catalogo-vpfe-hab.dian.gov.co/document/searchqr?documentkey=" + str(CUFE)
        ...
        if DocType == "FV" and Contingency == "N":
            QR = ('NumFac=' + SeriePrefix + SerieNumber + ' FecFac=' + IssueDate + ' HorFac=' + IssueTime +
                  ...
                  ' ValTolDocSop=' + str(TotalVal) + ' CUFE=' + str(CUFE) + " QRCode=" + QRCode)
        else:
            QR = ('NumFac=' + SeriePrefix + SerieNumber + ' FecFac=' + IssueDate + ' HorFac=' + IssueTime +
                  ' NitFac=' + IssuerNit + ' DocAdq=' + CustomerDocumentNumber + ' ValFac=' + str(InvoiceVal) +
                  ' ValIva=' + str(TotalIva) + ' ValOtroIm=' + str(TotalTaxConsume) + str(TotalIca) +
                  ' ValTolDocSop=' + str(TotalVal) + ' CUDE=' + str(CUFE) + " QRCode=" + QRCode)
        ...
        if DocType == "DS":
            Plane += '<cbc:ProfileID>DIAN 2.1: documento soporte en adquisiciones efectuadas a no obligados a facturar'\
                     '</cbc:ProfileID>'
            Plane += '<cbc:ProfileExecutionID>' + ProfileExecutionId + '</cbc:ProfileExecutionID>' \
                     '<cbc:ID>' + SeriePrefix + SerieNumber + '</cbc:ID>'
        else:
            if DocType == "FV":
                Plane += '<cbc:ProfileID>DIAN 2.1: Factura Electrónica de Venta</cbc:ProfileID>'
                Plane += '<cbc:ProfileExecutionID>' + ProfileExecutionId + '</cbc:ProfileExecutionID>' \
                         '<cbc:ID>' + SeriePrefix + SerieNumber + '</cbc:ID>'
            else:
                if DocType == "NC":
        ...
        if DocType == "DS" and Contingency == "N":
            Plane += '<cbc:UUID schemeID= "' + ProfileExecutionId + '" schemeName="CUDS-SHA384">' + str(CUFE) + \
                     '</cbc:UUID>'
        else:
            if DocType == "DS" and Contingency == "S":
                Plane += '<cbc:UUID schemeID= "' + ProfileExecutionId + '" schemeName="CUDS-SHA384">' + str(CUFE) + \
                         '</cbc:UUID>'

        if DocType == "FV" and Contingency == "N":
            Plane += '<cbc:UUID schemeID= "' + ProfileExecutionId + '" schemeName="CUFE-SHA384">' + str(CUFE) + \
                     '</cbc:UUID>'
        else:
            if DocType == "FV" and Contingency == "S":
                Plane += '<cbc:UUID schemeID= "' + ProfileExecutionId + '" schemeName="CUFE-SHA384">' + str(CUFE) + \
                         '</cbc:UUID>'
        ...
        if DocType == "DS" and Contingency == "N":
            Plane += '<cbc:InvoiceTypeCode>05</cbc:InvoiceTypeCode>'
            Plane += '<cbc:Note>' + NoteDocument.replace("&", "y") + '</cbc:Note>' \
            ...
            if DocType == "DS" and Contingency == "S":
                Plane += '<cbc:InvoiceTypeCode>03</cbc:InvoiceTypeCode>'
                Plane += '<cbc:Note>' + NoteDocument.replace("&", "y") + '</cbc:Note>' \
                         '<cbc:DocumentCurrencyCode listAgencyID="6" ' \
                         'listAgencyName="United Nations Economic Commission for Europe" listID="ISO 4217 Alpha">' + \
                         Currency + '</cbc:DocumentCurrencyCode>'
        ...
        if DocType == "FV" and Contingency == "N":
            Plane += '<cbc:InvoiceTypeCode>01</cbc:InvoiceTypeCode>'
            Plane += '<cbc:Note>' + NoteDocument.replace("&", "y") + '</cbc:Note>' \
                     '<cbc:DocumentCurrencyCode listAgencyID="6" ' \
                     'listAgencyName="United Nations Economic Commission for Europe" listID="ISO 4217 Alpha">' + \
                     Currency + '</cbc:DocumentCurrencyCode>'
        ...
        if "CreditNoteId" in new_data:
            if new_data["CreditNoteId"] != "":
                Plane += '<cac:BillingReference>' \
                         ...
                         '<cbc:UUID schemeName="CUDE-SHA384">' + new_data["CreditNoteCude"] + '</cbc:UUID>' \
        ...
        if "AdditionalDocumentReferenceId" in new_data:
            if new_data["AdditionalDocumentReferenceId"] != "":
                Plane += '<cac:AdditionalDocumentReference>' \
                         ...
                         '<cbc:DocumentTypeCode>' + new_data["AdditionalDocumentTypeCode"] + '</cbc:DocumentTypeCode>' \
        ...
        Plane += '</cac:TaxScheme>' \
                 ...
                 '<cbc:RegistrationName>' + IssuerName.replace("&", "y") + '</cbc:RegistrationName>' \
                 '<cbc:CompanyID schemeAgencyID="195" ' \
                 'schemeAgencyName="CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)" ' \
        ...
        if IssuerCommercialRegistration != "":
            Plane += '<cbc:Name>' + IssuerCommercialRegistration.replace("&", "y") + '</cbc:Name>'
        Plane += '</cac:CorporateRegistrationScheme>' \
                 '</cac:PartyLegalEntity>' \
                 '<cac:Contact>' \
                 '<cbc:Name>' + IssuerContactName.replace("&", "y") + '</cbc:Name>' \
        ...
        if CustomerLegalType == "2":
            ...
            Plane += '</cac:PartyIdentification>'
        Plane += '<cac:PartyName>' \
                 '<cbc:Name>' + CustomerName.replace("&", "y") + '</cbc:Name>' \
                 '</cac:PartyName>' \
                 '<cac:PhysicalLocation>' \
                 '<cac:Address>' \
                 ...
                 '<cac:AddressLine>' \
                 '<cbc:Line>' + CustomerAddressLine.replace("&", "y") + '</cbc:Line>' \
                 '</cac:AddressLine>' \
                 '<cac:Country>' \
                 '<cbc:IdentificationCode>CO</cbc:IdentificationCode>' \
                 '<cbc:Name languageID="es">Colombia</cbc:Name>' \
                 ...
                 '</cac:RegistrationAddress>' \
                 '<cac:TaxScheme>'
        for TaxC in CustomerTaxScheme:
            Plane += '<cbc:ID>' + get_tax_scheme(TaxC["TaxCategory"]) + '</cbc:ID>' \
                                                                        '<cbc:Name>' + get_tax_scheme_name(
                TaxC["TaxCategory"]) + '</cbc:Name>'

        Plane += '</cac:TaxScheme>' \
                 '</cac:PartyTaxScheme>' \
                 '<cac:PartyLegalEntity>' \
                 '<cbc:RegistrationName>' + CustomerCommercialRegistration.replace("&", "y") + '</cbc:RegistrationName>' \
                 '<cbc:CompanyID schemeAgencyID="195" ' \
                 'schemeAgencyName="CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)" ' \
                 'schemeID= "' + CustomerCheckDigit + '" schemeName= "' + CustomerDocumentType + '">' + \
                 CustomerDocumentNumber + '</cbc:CompanyID>'
        ...
        if CustomerDocumentType != "":
            Plane += '<cac:CorporateRegistrationScheme><cbc:Name>' + CustomerName.replace("&", "y") + '</cbc:Name>'
            Plane += '</cac:CorporateRegistrationScheme>'
        Plane += '</cac:PartyLegalEntity>' \
                 '<cac:Contact>' \
                 '<cbc:Name>' + CustomerContactName.replace("&", "y") + '</cbc:Name>' \
                 ...
                 'schemeAgencyName="CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)" ' \
                 'schemeID="4" schemeName="31">989123123</cbc:ID>' \
                 '</cac:PartyIdentification>' \
                 '</cac:TaxRepresentativeParty>'
        if DeliveryAddress != "":
            Plane += '<cac:Delivery>' \
        ...
        if DeliveryPartyTaxScheme != None:
            for TaxD in DeliveryPartyTaxScheme:
                Plane += '<cbc:ID>' + get_tax_scheme(TaxD["TaxCategory"]) + '</cbc:ID>' \
                         '<cbc:Name>' + get_tax_scheme_name(TaxD["TaxCategory"]) + '</cbc:Name>' \
                         '</cac:TaxScheme>' \
                         '</cac:PartyTaxScheme>' \
                         '<cac:PartyLegalEntity>' \
        ...
        if TaxHoldingTotalGlobal != None:
            for Tax in TaxHoldingTotalGlobal:
                TaxFilter = []
                for xtax in TaxHoldingSubTotalGlobal:
                    if Tax['WithholdingTaxCategory'] == xtax["WithholdingTaxCategory"]:
                        TaxFilter.append(xtax)
                Plane += '<cac:WithholdingTaxTotal>' \
                         '<cbc:TaxAmount currencyID="' + Currency + '">' + str(Tax["TaxAmount"]) + '</cbc:TaxAmount>' \
                         '<cbc:RoundingAmount currencyID="' + Currency + '">0.00</cbc:RoundingAmount>'
                for xtax in TaxFilter:
                    Plane += '<cac:TaxSubtotal>' \
                             ...
                             '<cbc:Percent>' + str(xtax["TaxPercentage"]) + '</cbc:Percent>' \
                             '<cac:TaxScheme>' \
                             '<cbc:ID>' + get_tax_scheme(xtax["WithholdingTaxCategory"]) + '</cbc:ID>' \
                             '<cbc:Name>' + get_tax_scheme_name(xtax["WithholdingTaxCategory"]) + '</cbc:Name>' \
                             '</cac:TaxScheme>' \
                             '</cac:TaxCategory>' \
                             '</cac:TaxSubtotal>'
            Plane += '</cac:WithholdingTaxTotal>'
        ...
        if DocType != "ND":
            Plane += '<cac:LegalMonetaryTotal>' \
                     '<cbc:LineExtensionAmount currencyID="' + Currency + '">' + str(InvoiceVal) + \
                     '</cbc:LineExtensionAmount>' \
                     '<cbc:TaxExclusiveAmount currencyID="' + Currency + '">' + str(TaxExclusiveAmount) + \
            ...
            Plane += '<cac:RequestedMonetaryTotal>' \
                     '<cbc:LineExtensionAmount currencyID="COP">' + str(InvoiceVal) + '</cbc:LineExtensionAmount>' \
                     '<cbc:TaxExclusiveAmount currencyID="' + Currency + '">' + str(TaxExclusiveAmount) + \
                     '</cbc:TaxExclusiveAmount>' \
                     '<cbc:TaxInclusiveAmount currencyID="' + Currency + '">' + str(TotalInvoiceBeforeDiscount) + \
                     '</cbc:TaxInclusiveAmount>' \
    ...
    def Create_xml_ad(new_data: dict, sends: dict, NumberContainer: int):
        ...
        xmlad += '<cbc:ProfileExecutionID>' + ProfileExecutionId + '</cbc:ProfileExecutionID>' \
                 ...
                 '<cbc:CompanyID schemeAgencyID = "195" schemeID = "' + IssuerNit + '" schemeName = "' + \
                 IssuerCheckDigit + '" schemeAgencyName = "CO,DIAN(Dirección de Impuestos y Aduanas Nacionales)">' + \
                 IssuerNit + '</cbc:CompanyID>' \
    ...
    @staticmethod
    def Create_xml_ad_ds(new_data: dict, sends: dict, NumberContainer: int):
        DocType = new_data["DocType"]
        SeriePrefix = new_data["SeriePrefix"]
        SerieNumber = new_data["SerieNumber"]
        ProfileExecutionId = new_data["ProfileExecutionId"]
        ...
        IssueDate = new_data["IssueDate"]
        IssueTime = new_data["IssueTime"]
        Issuer = new_data["AccountingCustomerParty"]
        IssuerResponsabilityTypes = Issuer["ResponsabilityTypes"]
        IssuerCommercialRegistration = Issuer["CommercialRegistration"]
        IssuerCheckDigit = Issuer["CheckDigit"]
        CUFE = new_data["CUDS"]
        Customer = new_data["AccountingSupplierParty"]
        CustomerDocumentType = Customer["DocumentType"]
        CustomerDocumentNumber = Customer["DocumentNumber"]
        CustomerCommercialRegistration = Customer["CommercialRegistration"]
        CustomerResponsabilityTypes = Customer["ResponsabilityTypes"]
        ...
        xmlad += '<cbc:ProfileExecutionID>' + ProfileExecutionId + '</cbc:ProfileExecutionID>' \
                 ...
                 '<cac:PartyTaxScheme>' \
                 '<cbc:RegistrationName>' + CustomerCommercialRegistration + '</cbc:RegistrationName>' \
                 '<cbc:CompanyID schemeAgencyID = "195" schemeID = "' + CustomerDocumentNumber + '" schemeName = "' + \
                 CustomerDocumentType + '" schemeAgencyName = "CO,DIAN(Dirección de Impuestos y Aduanas Nacionales)">' \
                 + CustomerDocumentNumber + '</cbc:CompanyID>' \
                 '<cbc:TaxLevelCode listName = "48">' + CustomerResponsabilityTypes + '</cbc:TaxLevelCode>' \
                 '<cac:TaxScheme>' \
                 '<cbc:ID>01</cbc:ID>' \
    ...
    @staticmethod
    def Create_xml_ds(new_data: dict):
        ...
        IssuerNit = new_data["IssuerNit"]
        ProfileExecutionId = new_data["ProfileExecutionId"]  # 1 produccion 2 habilitacion
        CustomizationId = new_data["CustomizationId"]
        Currency = new_data["Currency"]
        OperationType = new_data["OperationType"]
        SoftwareId = new_data["DianSoftwareId"]
        Pin = new_data["DianPin"]
        ...
        IssuerCityCode = Issuer["CityCode"]
        IssuerCityName = Issuer["CityName"]
        IssuerPostalCode = Issuer["PostalZone"]
        IssuerDepartmentCode = Issuer["DepartmentCode"]
        IssuerAddressLine = Issuer["AddressLine"]
        IssuerDepartment = Issuer["Department"]
        IssuerCommercialRegistration = Issuer["CommercialRegistration"]
        ...
        CustomerCityCode = Customer["CityCode"]
        CustomerCityName = Customer["CityName"]
        CustomerPostalCode = Customer["PostalZone"]
        CustomerDepartmentCode = Customer["DepartmentCode"]
        CustomerAddressLine = Customer["AddressLine"]
        CustomerDepartment = Customer["Department"]
        CustomerResponsabilityTypes = Customer["ResponsabilityTypes"]
        ...
        TaxExclusiveAmount = new_data["TaxExclusiveAmount"]
        TotalInvoiceBeforeDiscount = new_data["TaxInclusiveAmount"]
        GlobalDiscount = new_data["AllowanceTotalAmount"]
        TotalVal = new_data["PayableAmount"]
        TotalIva = new_data["TotalIva"]
        TotalTaxConsume = new_data["TotalTaxConsume"]
        TotalIca = new_data["TotalIca"]
        ...
        if new_data['CUDS'] == '':
            if new_data["CUDS"] == "":
                CUDS = (SeriePrefix + SerieNumber + IssueDate + IssueTime + str(InvoiceVal) + '01' + str(TotalIva) +
                        str(TotalVal) + CustomerDocumentNumber + IssuerNit + Pin + ProfileExecutionId)
                h = hashlib.sha384()
                h.update(CUDS.encode('utf-8'))
                CUDS = h.hexdigest()
            else:
                CUDS = new_data["CUDS"]
        ...
        if DocType == "DS":
            if ProfileExecutionId == "1":
                QRCode = "https://catalogo-vpfe.dian.gov.co/document/searchqr?documentkey=" + str(CUDS)
            else:
                QRCode = "https://catalogo-vpfe-hab.dian.gov.co/document/searchqr?documentkey=" + str(CUDS)
        else:
            if ProfileExecutionId == "1":
                QRCode = "https://catalogo-vpfe.dian.gov.co/document/searchqr?documentkey=" + str(CUDS)
            else:
        ...
        if DocType == "DS":
            QR = "N°DocSoporte=" + SeriePrefix + SerieNumber + " Fecha=" + IssueDate + " Hora=" + IssueTime + \
                 " ValDS=" + str(InvoiceVal) + " CodImp=01" + " ValImp=" + str(TotalIva) + \
                 " ValTot=" + str(TotalVal) + " NumSNO=" + CustomerDocumentNumber + " NITABS=" + IssuerNit + \
                 " PIN=" + Pin + " Amb=" + ProfileExecutionId + " CUDS=" + str(CUDS) + " URL=" + QRCode
        else:
            QR = "N°NotaAjuste=" + SeriePrefix + SerieNumber + " Fecha=" + IssueDate + " Hora=" + IssueTime + \
                 " ValDS=" + str(InvoiceVal) + " CodImp=01" + " ValImp=" + str(TotalIva) + \
                 " ValTot=" + str(TotalVal) + " NumSNO=" + CustomerDocumentNumber + " NITABS=" + IssuerNit + \
                 " PIN:" + Pin + " Amb:" + ProfileExecutionId + " CUDS=" + str(CUDS) + " URL=" + QRCode
        ...
        if True:  # factura
            for det in details:
                ...
                PriceAmount = det["PriceAmount"]
                InvoicePeriod = {}
                if "InvoicePeriod" in det:
                    InvoicePeriod = det["InvoicePeriod"]
                if 'CommercialPrice' in det:
                    CommercialPrice = det["CommercialPrice"]
                if 'BaseQuantity' in det:
                    BaseQuantity = det["BaseQuantity"]
                ...
                if InvoicePeriod:
                    Lines += '<cac:InvoicePeriod>' \
                             '<cbc:StartDate>' + InvoicePeriod["StartDate"] + '</cbc:StartDate>' \
                                                                              '<cbc:DescriptionCode>' + \
                             InvoicePeriod["DescriptionCode"] + '</cbc:DescriptionCode>'
                    if InvoicePeriod["DescriptionCode"] == "1":
                        Lines += '<cbc:Description>Por operación</cbc:Description>'
                    if InvoicePeriod["DescriptionCode"] == "2":
                        Lines += '<cbc:Description>Acumulado semanal</cbc:Description>'
                ...
                if WithHoldingTaxTotalLines != None:
                    for tax in WithHoldingTaxTotalLines:
                        ...
                        for wtax in WithHoldingTaxSubTotalLines:
                            Lines += '<cac:TaxSubtotal>' \
                                     '<cbc:TaxableAmount currencyID = "' + Currency + '">' + str(wtax["TaxableAmount"])\
                                     + '</cbc:TaxableAmount>' \
                                     '<cbc:TaxAmount currencyID = "' + Currency + '">' + str(wtax["TaxAmount"]) + \
                                     '</cbc:TaxAmount>' \
                                     '<cac:TaxCategory>' \
                ...
                for item in StandardItemIdentifications:
                    Lines += '<cac:StandardItemIdentification>' \
                             '<cbc:ID schemeAgencyID="10" schemeID="999" ' \
                             'schemeName="Estándar de adopción del contribuyente">' + \
                             (item["SchemaId"]) + '</cbc:ID>' \
                             '</cac:StandardItemIdentification>'
                Lines += '</cac:Item>'
        ...
        if DocType == "DS":
            ...
            if DocType == "FV":
                ...
                if DocType == "NDS":
                    Plane = '<CreditNote xmlns="urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2" ' \
                            'xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" ' \
                            'xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" ' \
                            'xmlns:ds="http://www.w3.org/2000/09/xmldsig#" ' \
                            'xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2" ' \
        ...
        if DocType == "DS":
            ...
            if DocType == "NDS":
                Plane += '<sts:InvoiceSource>' \
                         '<cbc:IdentificationCode listAgencyID = "6" ' \
                         'listAgencyName = "United Nations Economic Commission for Europe" ' \
                         'listSchemeURI = "urn:oasis:names:specification:ubl:codelist:gc:' \
                         'CountryIdentificationCode-2.1">CO</cbc:IdentificationCode>' \
                         '</sts:InvoiceSource>'
        ...
        Plane += '<cbc:CustomizationID>' + CustomizationId + '</cbc:CustomizationID>'

        if DocType == "DS":
            Plane += '<cbc:ProfileID>DIAN 2.1: documento soporte en adquisiciones efectuadas a no obligados a ' \
                     'facturar.</cbc:ProfileID>'
            Plane += '<cbc:ProfileExecutionID>' + ProfileExecutionId + '</cbc:ProfileExecutionID>' \
                     '<cbc:ID>' + SeriePrefix + SerieNumber + '</cbc:ID>'
        else:
            if DocType == "NDS":
                Plane += '<cbc:ProfileID>DIAN 2.1: Nota de ajuste al documento soporte en adquisiciones efectuadas a ' \
                         'sujetos no obligados a expedir factura o documento equivalente</cbc:ProfileID>'
                Plane += '<cbc:ProfileExecutionID>' + ProfileExecutionId + '</cbc:ProfileExecutionID>' \
                         '<cbc:ID>' + SeriePrefix + SerieNumber + '</cbc:ID>'

        if DocType == "DS":
            Plane += '<cbc:UUID schemeID= "' + ProfileExecutionId + '" schemeName="CUDS-SHA384">' + str(
                CUDS) + '</cbc:UUID>'
        else:
            if DocType == "DS" and Contingency == "S":
                Plane += '<cbc:UUID schemeID= "' + ProfileExecutionId + '" schemeName="CUDS-SHA384">' + \
                         str(CUDS) + '</cbc:UUID>'
            else:
                if DocType == "NDS":
                    Plane += '<cbc:UUID schemeID= "' + ProfileExecutionId + '"  schemeName="CUDS-SHA384">' + \
        ...
        if DocType == "DS":
            ...
            if DocType == "DS" and Contingency == "S":
                ...
                if DocType == "NDS":
                    Plane += '<cbc:CreditNoteTypeCode>95</cbc:CreditNoteTypeCode>'
                    Plane += '<cbc:Note>' + NoteDocument + '</cbc:Note>' \
                             '<cbc:DocumentCurrencyCode>' + Currency + '</cbc:DocumentCurrencyCode>'

        if DocType == "DS" or (DocType == "ND" and ProfileExecutionId == "2"):
            Plane += '<cbc:LineCountNumeric>' + LineCount + '</cbc:LineCountNumeric>'
        else:
            Plane += '<cbc:LineCountNumeric>' + str(Nline - 1) + '</cbc:LineCountNumeric>'
        if DocType == "DS":
        ...
        Plane += '<cac:AccountingSupplierParty>' \
                 '<cbc:AdditionalAccountID>' + CustomerLegalType + '</cbc:AdditionalAccountID>' \
                 '<cac:Party>' \
                 '<cac:PartyName>' \
                 ...
                 '</cbc:CompanyID>' \
                 '<cbc:TaxLevelCode listName="05">' + CustomerResponsabilityTypes + '</cbc:TaxLevelCode>' \
                 '<cac:RegistrationAddress>' \
                 '<cbc:ID>' + CustomerCityCode + '</cbc:ID>' \
                 '<cbc:CityName>' + CustomerCityName + '</cbc:CityName>' \
                 '<cbc:PostalZone>' + CustomerPostalCode + '</cbc:PostalZone>' \
        ...
        Plane += '</cac:TaxScheme>' \
                 ...
                 '<cac:CorporateRegistrationScheme>' \
                 '<cbc:ID>' + SeriePrefix + '</cbc:ID>'
        if CustomerCommercialRegistration != "":
            Plane += '<cbc:Name>' + CustomerCommercialRegistration + '</cbc:Name>'
        Plane += '</cac:CorporateRegistrationScheme>' \
                 '</cac:PartyLegalEntity>' \
                 '<cac:Contact>' \
                 ...
                 '<cbc:Telephone>' + CustomerContactTelephone + '</cbc:Telephone>' \
                 '<cbc:ElectronicMail>' + CustomerContactElectronicMail + '</cbc:ElectronicMail>' \
                 '<cbc:Note>' + CustomerContactName + '</cbc:Note>' \
                 '</cac:Contact>' \
                 '</cac:Party>' \
                 '</cac:AccountingSupplierParty>'
        Plane += '<cac:AccountingCustomerParty>'
        Plane += '<cbc:AdditionalAccountID>' + IssuerLegalType + '</cbc:AdditionalAccountID>'
        Plane += '<cac:Party>'

        if IssuerLegalType == "2":
            Plane += '<cac:PartyIdentification>' \
                     '<cbc:ID '
            if IssuerDocumentType == "31":
                Plane += 'schemeID="' + IssuerCheckDigit + '" '
            Plane += 'schemeName="' + IssuerDocumentType + '">' + IssuerNit + '</cbc:ID>'
            Plane += '</cac:PartyIdentification>'
        Plane += '<cac:PartyName>' \
                 '<cbc:Name>' + IssuerName + '</cbc:Name>' \
                 '</cac:PartyName>' \
                 ...
                 '<cbc:ID>' + IssuerCityCode + '</cbc:ID>' \
                 '<cbc:CityName>' + IssuerCityName + '</cbc:CityName>' \
                 '<cbc:PostalZone>' + IssuerPostalCode + '</cbc:PostalZone>' \
                 '<cbc:CountrySubentity>' + IssuerDepartment + '</cbc:CountrySubentity>' \
                 '<cbc:CountrySubentityCode>' + IssuerDepartmentCode + '</cbc:CountrySubentityCode>' \
                 '<cac:AddressLine>' \
                 '<cbc:Line>' + IssuerAddressLine + '</cbc:Line>' \
                 ...
                 'schemeAgencyName= "CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)" schemeID= "' + \
                 IssuerCheckDigit + '" schemeName= "' + IssuerDocumentType + '">' + IssuerNit + '</cbc:CompanyID>' \
                 '<cbc:TaxLevelCode listName="04">' + IssuerResponsabilityTypes + '</cbc:TaxLevelCode>' \
                 '<cac:RegistrationAddress>' \
                 '<cbc:ID>' + IssuerCityCode + '</cbc:ID>' \
                 '<cbc:CityName>' + IssuerCityName + '</cbc:CityName>' \
                 '<cbc:CountrySubentity>' + IssuerDepartment + '</cbc:CountrySubentity>' \
        ...
        for TaxC in IssuerTaxScheme:
            Plane += '<cbc:ID>' + get_tax_scheme(TaxC["TaxCategory"]) + '</cbc:ID>' \
                     '<cbc:Name>' + get_tax_scheme_name(TaxC["TaxCategory"]) + '</cbc:Name>'
        ...
        Plane += '</cac:TaxScheme>' \
                 ...
                 '<cbc:CompanyID schemeAgencyID="195" ' \
                 'schemeAgencyName="CO, DIAN (Dirección de Impuestos y Aduanas Nacionales)" schemeID= "' + \
                 IssuerCheckDigit + '" schemeName= "' + IssuerDocumentType + '">' + IssuerNit + \
                 '</cbc:CompanyID>'

        if IssuerDocumentType != "":
            Plane += '<cac:CorporateRegistrationScheme><cbc:Name>' + IssuerName + '</cbc:Name>'
            Plane += '</cac:CorporateRegistrationScheme>'
        Plane += '</cac:PartyLegalEntity>' \
                 '<cac:Contact>' \
                ...
                for Tax in TaxHoldingTotalGlobal:
                    ...
                    for xtax in TaxHoldingSubTotalGlobal:
                        if Tax['WithholdingTaxCategory'] == xtax["WithholdingTaxCategory"]:
                            TaxFilter.append(xtax)
                    Plane += '<cac:WithholdingTaxTotal>' \
                             '<cbc:TaxAmount currencyID="' + Currency + '">' + str(
                        Tax["TaxAmount"]) + '</cbc:TaxAmount>' \
                                            '<cbc:RoundingAmount currencyID="' + Currency + '">0.00</cbc:RoundingAmount>'
                    for xtax in TaxFilter:
                        Plane += '<cac:TaxSubtotal>' \
                                 '<cbc:TaxableAmount currencyID="' + Currency + '">' + str(xtax["TaxableAmount"]) + \
                                 ...
                                 '<cbc:Percent>' + str(xtax["TaxPercentage"]) + '</cbc:Percent>' \
                                                                                '<cac:TaxScheme>' \
                                                                                '<cbc:ID>' + get_tax_scheme(
                            xtax["WithholdingTaxCategory"]) + '</cbc:ID>' \
                                                              '<cbc:Name>' + get_tax_scheme_name(
                            xtax["WithholdingTaxCategory"]) + '</cbc:Name>' \
                                                              '</cac:TaxScheme>' \
                                                              '</cac:TaxCategory>' \
                                                              '</cac:TaxSubtotal>'
                Plane += '</cac:WithholdingTaxTotal>'
        ...
        if DocType != "ND":
            Plane += '<cac:LegalMonetaryTotal>' \
                     ...
                     '</cbc:TaxInclusiveAmount>'
            if GlobalDiscount != "0":
                   Plane +=  '<cbc:AllowanceTotalAmount currencyID="' + Currency + '">' + str(GlobalDiscount) + \
                     '</cbc:AllowanceTotalAmount>'
            Plane += '<cbc:PayableAmount currencyID="' + Currency + '">' + str(TotalVal) + '</cbc:PayableAmount>' \
                     '</cac:LegalMonetaryTotal>'
