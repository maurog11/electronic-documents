...
class ConsecutiveNotesModel(BaseModel):
    __tablename__ = 'consecutivenotes'

    IssuerNit = db.Column(db.String(20), unique=False, nullable=False, comment='Nit ')
    DocType = db.Column(db.String(5), nullable=False, unique=False, comment='NC ND NCDS ')
    ...
    def get_data(export: bool = False):
        try:
            response = db.session.query(ConsecutiveNotesModel) \
                .all()
            if export:
                response = [ConsecutiveNotesModel.export_data(x) for x in response]
            return response
    ...
    def get_by_id(id: int, export: bool = False):
        try:
            response = db.session.query(ConsecutiveNotesModel) \
                .filter(ConsecutiveNotesModel.id == id) \
                .first()
            if export and response:
                response = ConsecutiveNotesModel.export_data(response)
            return response
        except Exception as e:
            print(e)
            raise InternalServerError(e)
