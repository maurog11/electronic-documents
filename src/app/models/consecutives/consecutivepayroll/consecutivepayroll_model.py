...
class ConsecutivePayRollModel(BaseModel):
    __tablename__ = 'consecutivepayroll'

    IssuerNit = db.Column(db.String(20), unique=False, nullable=False, comment='Nit ')
    DocType = db.Column(db.String(5), nullable=False, unique=False, comment='NO NA ')
    ...
    def get_data(export: bool = False):
        try:
            response = db.session.query(ConsecutivePayRollModel) \
                .all()
            if export:
                response = [ConsecutivePayRollModel.export_data(x) for x in response]
            return response
    ...
    def get_by_id(id: int, export: bool = False):
        try:
            response = db.session.query(ConsecutivePayRollModel) \
                .filter(ConsecutivePayRollModel.id == id) \
                .first()
            if export and response:
                response = ConsecutivePayRollModel.export_data(response)
            return response
        except Exception as e:
            print(e)
            raise InternalServerError(e)
